    ...
    public class RecordEmailDeliveredCommandHandler : IRequestHandler<RecordEmailDeliveredCommand, Result<bool>>
    {
        ...
        public async Task<Result<bool>> Handle(RecordEmailDeliveredCommand request, CancellationToken cancellationToken)
        {
            ...
            _context.EmailEventLogs.Add(new EmailEventLog
            {
                ...
                EventType = EmailEventLogType.Delivered.ToString(),
                LogData = JsonSerializer.Serialize(eventData),
                EventLogDate = deliveredAt,
                Status = 1,
                CreatedBy = "System",
                CreatedDate = now
            });
            ...
            await _webhookDispatcher.QueueWebhookAsync(emailLog.Id, EmailEventLogType.Delivered.ToString(), eventData, cancellationToken);
